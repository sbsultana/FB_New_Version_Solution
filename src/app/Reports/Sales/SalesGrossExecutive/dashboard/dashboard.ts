...
import { SalesgrossexecutiveReports } from '../salesgrossexecutive-reports/salesgrossexecutive-reports';
import { Subscription } from 'rxjs';
import { Sharedservice } from '../../../../Core/Providers/Shared/sharedservice';
import { Setdates } from '../../../../Core/Providers/SetDates/setdates';
import { SalesgrossDetails } from '../../SalesGross/salesgross-details/salesgross-details';

@Component({
  selector: 'app-dashboard',
  imports: [SharedModule, SalesgrossexecutiveReports],
  standalone: true,
...
export class Dashboard {
  ...
  constructor(public shared: Sharedservice, public setdates: Setdates) {
    ...
    this.shared.setTitle(this.shared.common.titleName + '-Sales Gross (Executive)')
  ...
  getPackTracker() {
    const obj = {
      "userid": JSON.parse(localStorage.getItem('userInfo')!)?.user_Info.ADuserid
    }
    this.shared.api.postmethodOne('users/AxelOneUserinfo', obj).subscribe((res: any) => {
      if (res && res.response) {
        this.packstatus = res.response.Pack_tracker
  ...
  GetData() {
    ...
    const curl = this.shared.getEnviUrl() + this.shared.common.routeEndpoint + 'GetSalesGrossExecutive';
    this.shared.api
      .postmethod(this.shared.common.routeEndpoint + 'GetSalesGrossExecutive', obj)
      .subscribe(
        (res) => {
  ...
  ngAfterViewInit() {
    ...
    this.shared.api.GetReports().subscribe((data) => {
      ...
      // console.log(data);
      if (data.obj.Reference == 'Sales Gross (Executive)') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        if (res.obj.title == 'Sales Gross (Executive)') {
  ...
  exportToExcel(): void {
    ...
    const worksheet = workbook.addWorksheet('Sales Gross (Executive)');
    ...
    const titleRow = worksheet.addRow(['Sales Gross (Executive)']);
   ...
   worksheet.addRow('');
    ...
    if (this.packstatus == 'Y') {
      worksheet.mergeCells('V12:X12');
      worksheet.mergeCells('Y12:AA12');
    }
    ...
    if (this.packstatus == 'Y') {
      worksheet.getCell('V12').value = 'PACK GROSS';
      worksheet.getCell('Y12').value = 'TOTALS W/PACKS';
    }
    ...
    if (this.packstatus == 'Y') {
      ['A12', 'B12', 'G12', 'L12', 'Q12', 'V12', 'Y12'].forEach(key => {
    ...
    var secondHeader: any = []


    if (this.packstatus == 'Y') {
      secondHeader = [
        `${FromDate} - ${ToDate}, ${PresentYear}`,
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'TARGET', '+/-', 'PER DAY',
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'TARGET', '+/-', 'PVR',
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'TARGET', '+/-', 'PVR',
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'TARGET', '+/-', 'PVR',
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'PVR',
        this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'PACE', 'PVR',
    ...
    if (this.packstatus == 'Y') {
      bindingHeaders = [
        ...
        'PackGross_MTD', 'PackGross_Pace', 'PackGross_PVR',
        'TP_MTD', 'TP_PACE', 'TP_PVR'
      ];
    }
    else {
      bindingHeaders = [
    ...
    const currencyFields = [
      'FrontGross_MTD', 'FrontGross_Pace', 'FrontGross_Target', 'FrontGross_Diff', 'FrontGross_PVR',
      'BackGross_MTD', 'BackGross_Pace', 'BackGross_Target', 'BackGross_Diff', 'BackGross_PVR',
      'PackGross_MTD', 'PackGross_Pace', 'PackGross_PVR', 'TP_MTD', 'TP_PACE', 'TP_PVR',
      'TotalGross_MTD', 'TotalGross_Pace', 'TotalGross_Target', 'TotalGross_Diff', 'TotalGross_PVR',];
    ...
    for (const info of this.SalesData) {
      const rowData = bindingHeaders.map((key: any) => {
        ...
        // if (key === 'PackGross_Diff' && info['PackGross_Pace'] != 0 && info['PackGross_Pace'] != null && info['PackGross_Target'] != 0 && info['PackGross_Target'] != null) return val;
        ...
        if (key === 'TP_MTD' && (info['TotalGross_MTD'] + info['PackGross_MTD']) != 0 && (info['TotalGross_MTD'] + info['PackGross_MTD']) != null) return (info['TotalGross_MTD'] + info['PackGross_MTD']);
        if (key === 'TP_PACE' && (info['TotalGross_Pace'] + info['PackGross_Pace']) != 0 && (info['TotalGross_Pace'] + info['PackGross_Pace']) != null) return (info['TotalGross_Pace'] + info['PackGross_Pace']);
        if (key === 'TP_PVR' && (info['TotalGross_PVR'] + info['PackGross_PVR']) != 0 && (info['TotalGross_PVR'] + info['PackGross_PVR']) != null) return (info['TotalGross_PVR'] + info['PackGross_PVR']);
      ...
      if (info.Data2 != undefined) {
        for (const data2 of info.Data2) {
          const nestedRowData = bindingHeaders.map((key: any) => {
            ...
            // if (key === 'PackGross_Diff' && data2['PackGross_Pace'] != 0 && data2['PackGross_Pace'] != null && data2['PackGross_Target'] != 0 && data2['PackGross_Target'] != null) return (data2['PackGross_Pace'] - data2['PackGross_Target']);
            ...
            if (key === 'PackGross_PVR') return (data2['PackGross_MTD'] / data2['Units_MTD']);

            if (key === 'TP_MTD' && (data2['TotalGross_MTD'] + data2['PackGross_MTD']) != 0 && (data2['TotalGross_MTD'] + data2['PackGross_MTD']) != null) return (data2['TotalGross_MTD'] + data2['PackGross_MTD']);
            if (key === 'TP_PACE' && (data2['TotalGross_Pace'] + data2['PackGross_Pace']) != 0 && (data2['TotalGross_Pace'] + data2['PackGross_Pace']) != null) return (data2['TotalGross_Pace'] + data2['PackGross_Pace']);
            if (key === 'TP_PVR' && ((data2['TotalGross_MTD'] / data2['Units_MTD']) + (data2['PackGross_MTD'] / data2['Units_MTD'])) != 0 && ((data2['TotalGross_MTD'] / data2['Units_MTD']) + (data2['PackGross_MTD'] / data2['Units_MTD'])) != null) return ((data2['TotalGross_MTD'] / data2['Units_MTD']) + (data2['PackGross_MTD'] / data2['Units_MTD']));
            const val = data2[key];
          ...
          if (data2.SubData != undefined) {
            for (const data3 of data2.SubData) {
              const nestedRowData = bindingHeaders.map((key: any) => {
                ...
                if (key === 'PackGross_PVR') return (data3['PackGross_MTD'] / data3['Units_MTD']);
                ...
                // if (key === 'PackGross_Target') return '';

                if (key === 'TotalGross_Target') return '';

                if (key === 'FrontGross_Diff') return '';
                if (key === 'BackGross_Diff') return '';
                // if (key === 'PackGross_Diff') return '';

                if (key === 'TotalGross_Diff') return '';

                if (key === 'TP_MTD' && (data3['TotalGross_MTD'] + data3['PackGross_MTD']) != 0 && (data3['TotalGross_MTD'] + data3['PackGross_MTD']) != null) return (data3['TotalGross_MTD'] + data3['PackGross_MTD']);
                if (key === 'TP_PACE' && (data3['TotalGross_Pace'] + data3['PackGross_Pace']) != 0 && (data3['TotalGross_Pace'] + data3['PackGross_Pace']) != null) return (data3['TotalGross_Pace'] + data3['PackGross_Pace']);
                if (key === 'TP_PVR' && ((data3['TotalGross_MTD'] / data3['Units_MTD']) + (data3['PackGross_MTD'] / data3['Units_MTD'])) != 0 && ((data3['TotalGross_MTD'] / data3['Units_MTD']) + (data3['PackGross_MTD'] / data3['Units_MTD'])) != null) return ((data3['TotalGross_MTD'] / data3['Units_MTD']) + (data3['PackGross_MTD'] / data3['Units_MTD']));
                const val = data3[key];
                return val === 0 || val == null ? '-' : val;
              });
              const nestedRow = worksheet.addRow(nestedRowData);
    ...
    workbook.xlsx.writeBuffer().then((buffer: any) => {
      ...
      this.shared.exportToExcel(workbook, 'Sales Gross Executive')
    });
  }
