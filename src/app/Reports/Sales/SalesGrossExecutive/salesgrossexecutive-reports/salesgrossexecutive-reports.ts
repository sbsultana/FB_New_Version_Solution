...
@Component({
  selector: 'app-salesgrossexecutive-reports',
  imports: [SharedModule, DateRangePicker, Stores,ToastContainer],
  templateUrl: './salesgrossexecutive-reports.html',
  styleUrl: './salesgrossexecutive-reports.scss',
})
export class SalesgrossexecutiveReports {
  ngChanges: any = []
    ...
    if (changes['header'].previousValue != undefined) {
      ...
      this.dataGrouping.forEach((ele: any) => {
        ...
        this.shared.api.GetHeaderData().subscribe((res) => {
          if (
            res.obj.title == 'Sales Gross (Executive)'
          ) {
  ...
  ngAfterViewInit() {
    this.shared.api.getStores().subscribe((res: any) => {
      ...
      if (this.shared.common.pageName == 'Sales Gross (Executive)') {
  ...
  viewreport() {
    ...
    if(!this.storeIds || this.storeIds.length === 0){
      ...
      const data = {
        Reference: 'Sales Gross (Executive)',
