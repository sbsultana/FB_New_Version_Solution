...
export class Dashboard {
  ...
  dataGrouping: any = [
    ...
    { "ARG_ID": 64, "ARG_LABEL": "Department", "ARG_SEQ": 1, "id": 64, "columnname": "ASG_Department", "Active": "Y" },
  ...
  constructor(
    ...
    this.shared.setTitle(this.comm.titleName + '-Service Gross GL');
    let today = new Date();
    if (today.getDate() == 1) {
  ...
  GetData() {
    ...
    const curl = environment.apiUrl + this.comm.routeEndpoint + 'GetServiceSummaryGLBeta';
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetServiceSummaryGLBeta', obj).subscribe(
      (res) => {
...
async openServiceModal(roNumber: any, vin: any, storeid: any, vehicleid: any, source: any, custno: any) {
  ...
  ngAfterViewInit() {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        if (res.obj.title == 'Service Gross GL') {
  ...
  exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet('Service Gross GL');
    worksheet.views = [
      {
        showGridLines: false,
      },
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Service Gross GL']);
      ...
      .subscribe((res) => {
        if (res.status == 200) {
          ...
          workbook.xlsx.writeBuffer().then((data: any) => {
            ...
            this.shared.exportToExcel(workbook, 'Service Gross GL Details')
          });
        }
      });
  }
