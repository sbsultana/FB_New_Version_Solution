...
export class Dashboard {
  ...
  constructor(
    ...
    this.shared.setTitle(this.comm.titleName + '-Fixed Summary GL');
    let today = new Date();
    if (today.getDate() == 1) {
  ...
  ngAfterViewInit() {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.comm.pageName == 'Fixed Summary GL') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        if (res.obj.title == 'Fixed Summary GL') {
  ...
  exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet('Fixed Summary GL');
    worksheet.views = [
      {
        showGridLines: false,
      },
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Fixed Summary GL']);
    ...
    let frontgross = worksheet.getCell('G11');
    ...
    let Headings = [
      ...
      'GL CNT',
      'Actual', 'Pace',
      //  'Forecast', 'Var', '% of Forecast', 
      this.shared.datePipe.transform(this.FromDate, 'MMM') +
      (this.shared.datePipe.transform(this.FromDate, 'MMM') != this.shared.datePipe.transform(this.ToDate, 'MMM') ? ('-' + this.shared.datePipe.transform(this.ToDate, 'MMM')) : '') + '-' +
      this.shared.datePipe.transform(this.lastyearDate, 'YYYY'),
      'Var', this.datetype() == 'C' ? this.shared.datePipe.transform(this.FromDate, 'MM.dd.yyyy') + '-' + this.shared.datePipe.transform(this.ToDate, 'MM.dd.yyyy') : this.datetype(), 'Pace', 'Forecast', 'Var', '% of Forecast',
      this.shared.datePipe.transform(this.FromDate, 'MMM') +
      (this.shared.datePipe.transform(this.FromDate, 'MMM') != this.shared.datePipe.transform(this.ToDate, 'MMM') ? ('-' + this.shared.datePipe.transform(this.ToDate, 'MMM')) : '') + '-' +
      this.shared.datePipe.transform(this.lastyearDate, 'YYYY'), 'Var', 'GP %'
    ];
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      this.shared.exportToExcel(workbook, 'Fixed Summary GL')
