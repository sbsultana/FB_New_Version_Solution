...
@Component({
  selector: 'app-inventory-open-rodetails',
   imports: [SharedModule,Notes],
  templateUrl: './inventory-open-rodetails.html',
  styleUrl: './inventory-open-rodetails.scss',
})
export class InventoryOpenRODetails {
  ...
  getMaxPageNumber(): number {
    ...
    return Math.ceil(this.filteredInventoryOpenROData.length / this.itemsPerPage);
  ...
  getDetails() {
    const obj = {
      ...
      var3Value: '',
      GrossTypeLabor: this.RODetailsObject[0].GrossTypeLabor,
      GrossTypeParts: this.RODetailsObject[0].GrossTypeParts,
      GrossTypeMisc: this.RODetailsObject[0].GrossTypeMisc,
      GrossTypeSublet: this.RODetailsObject[0].GrossTypeSublet,
      inventory: 'N',
      stocktype: this.RODetailsObject[0].inventory,
      PageNumber: 0,
      PageSize: this.total,
    ...
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetServicesGrossSummaryDetailsV1OpenInventory', obj).subscribe(
      (res) => {
        //console.log(res);
        if (res.status == 200) {
...
async openServiceModal(roNumber: any, vin: any, storeid: any, vehicleid: any, source: any, custno: any) {
  ...
  get paginatedItems() {
    ...
    return this.filteredInventoryOpenROData.slice(startIndex, endIndex);
  }
  QISearchName: any = ''
  filterData() {
    if (this.QISearchName.trim() !== '') {
      ...
      this.filteredInventoryOpenROData = this.ServiceData
        .map((item: any) => {
          ...
          const matchesVinNo = item.vin?.toLowerCase().includes(search);
    ...
    this.callLoadingState == 'ANS' ? this.sort(this.column, this.filteredInventoryOpenROData, this.callLoadingState) : ''
        ...
        if (res.status == 200) {
          ...
          if (res.response != undefined) {
            if (res.response.length > 0) {
              ...
              const worksheet = workbook.addWorksheet('Inventory Open RO  Details');
              worksheet.views = [
                {
                  state: 'frozen',
              ...
              const titleRow = worksheet.getCell("A2"); titleRow.value = 'Inventory Open RO  Details';
              ...
              for (const d of localarray) {
                ...
                const Data1 = worksheet.addRow([
                  ...
                  d.StockType == '' ? '-' : d.StockType == null ? '-' : d.StockType,
                  d.vehicle == '' ? '-' : d.vehicle == null ? '-' : d.vehicle,
                  d.vin == '' ? '-' : d.vin == null ? '-' : d.vin,
                  d.stock == '' ? '-' : d.stock == null ? '-' : d.stock,
                  d.Totalgross == '' ? '-' : d.Totalgross == null ? '-' : parseInt(d.Totalgross),
                  d.Totalcost == '' ? '-' : d.Totalcost == null ? '-' : parseInt(d.Totalcost),
                  d.TotalSale == '' ? '-' : d.TotalSale == null ? '-' : parseInt(d.TotalSale),
                  ...
                  d.Discount == '' ? '-' : d.Discount == null ? '-' : parseInt(d.Discount),
                  d.CustomerPayGross == '' ? '-' : d.CustomerPayGross == null ? '-' : parseInt(d.CustomerPayGross),
                  d.Retention == '' ? '-' : d.Retention == null ? '-' : parseFloat(d.Retention) + ' %',
                  ...
                  d.DiscountCP == '' ? '-' : d.DiscountCP == null ? '-' : parseInt(d.DiscountCP),
                  d.WarrantyGross == '' ? '-' : d.WarrantyGross == null ? '-' : parseInt(d.WarrantyGross),
                  ...
                  d.DiscountWP == '' ? '-' : d.DiscountWP == null ? '-' : parseInt(d.DiscountWP),
                  d.InternalGross == '' ? '-' : d.InternalGross == null ? '-' : parseInt(d.InternalGross),
                  ...
                  d.DiscountIP == '' ? '-' : d.DiscountIP == null ? '-' : parseInt(d.DiscountIP),
                ]);
                ...
                Data1.eachCell((cell, number) => {
                  cell.border = { right: { style: 'thin' } };
                  cell.numFmt = '$#,##0.00';
                  if (number == 14 || number == 19 || number == 24 || number == 29) {
                    cell.numFmt = '0.00';
                    cell.alignment = { indent: 1, vertical: 'middle', horizontal: 'center', };
                  }
                  if (number == 12 || number == 17 || number == 22 || number == 27) {
                    cell.numFmt = '0.0';
                    cell.alignment = { indent: 1, vertical: 'middle', horizontal: 'center', };
                  }
                ...
                if (d.NotesStatus == 'Y' && this.notesViewState == true) {
                  worksheet.mergeCells(notesCount, 1, notesCount, 30);
              ...
              worksheet.addRow([]);
                ...
                this.shared.exportToExcel(workbook, 'Inventory Open RO  Details' + DATE_EXTENSION);
            }
          }
        } else {
          this.shared.spinner.hide()
