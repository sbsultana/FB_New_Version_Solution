...
import { InventoryOpenRODetails } from '../inventory-open-rodetails/inventory-open-rodetails';
@Component({
  selector: 'app-dashboard',
  imports: [SharedModule, BsDatepickerModule, DateRangePicker, Stores, InventoryOpenRODetails, NgxSliderModule],
...
export class Dashboard {
  ...
  constructor(
    ...
    this.shared.setTitle(this.comm.titleName + '-Inventory Open RO');
  ...
  GetData() {
    ...
    const obj = {
      ...
      AdvisorName: '',
      ROSTATUS: '',
      Stocktype: this.inventory.toString() == 'All' ? '' : this.inventory.toString(),
      Department: '',
    ...
    const curl = environment.apiUrl + this.comm.routeEndpoint + 'GetServiceSummaryBetaOpenInventory';
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetServiceSummaryBetaOpenInventory', obj).subscribe(
      (res) => {
  ...
  ngAfterViewInit() {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.comm.pageName == 'Inventory Open RO') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        if (res.obj.title == 'Inventory Open RO') {
  ...
  exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet('Inventory Open RO');
    worksheet.views = [
      {
        showGridLines: false,
      },
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Inventory Open RO']);
    ...
    this.shared.exportToExcel(workbook, 'Inventory Open RO');
