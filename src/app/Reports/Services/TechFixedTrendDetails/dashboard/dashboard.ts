...
import { TechFixedTrendGraph } from '../tech-fixed-trend-graph/tech-fixed-trend-graph';
@Component({
...
export class Dashboard {
  ...
  date: any = ''
  TechFixedTrendingData: any;
  TechFixedTrendTotalsData: any = [];
  Month: any;
  ...
  constructor(public shared: Sharedservice, public setdates: Setdates, private comm: common, private cp: CurrencyPipe, private toast: ToastService,) {
    ...
    this.shared.setTitle(this.comm.titleName + '-Tech Fixed Trend Details');
    this.setHeaderData()
  ...
  setHeaderData() {
    const data = {
      title: 'Tech Fixed Trend Details',
      Month: this.date,
  ...
  dates: any;
  storeName: any;
  TechFixedTrendingKeys: any = [];
  ServiceTrendingSubKeys: any = [];
  ServiceTrendingXlm: any = [];
  GetDataByMonths() {
    this.dates = [];
    this.TechFixedTrendingData = [];
    ...
    console.log(obj);
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetTechFixedTrendDetails', obj).subscribe(
      (x: any) => {
        if (x.status == 200) {
          // .sort((a:any, b:any) => b.ast_paytype.localeCompare(a.ast_paytype))
          this.TechFixedTrendingData = x.response;
          const TechFixedTrendKeys = Object.keys(x.response[0]).slice(6);
          this.TechFixedTrendingKeys = TechFixedTrendKeys;
          console.log(this.TechFixedTrendingKeys, this.TechFixedTrendingData);
          this.TechFixedTrendingData = x.response.reduce(
          ...
          this.TechFixedTrendingData.forEach((e: any) => {
          ...
          if (this.TechFixedTrendingKeys && this.TechFixedTrendingKeys.length > 0) {
            this.NoData = false;
            this.shared.spinner.hide();
          } else {
            this.NoData = true;
        ...
        if (x.status == 200) {
          if (x.response != undefined) {
            if (x.response.length > 0) {
              this.TechFixedTrendTotalsData = x.response;
              const serviceKeys = Object.keys(x.response[0]).slice(4);
              this.TechFixedTrendingKeys = serviceKeys;
              console.log(this.TechFixedTrendingKeys);
              this.TechFixedTrendTotalsData = x.response.reduce(
                (r: any, { Store_Name }: any) => {
                  if (!r.some((o: any) => o.Store_Name == Store_Name)) {
                    r.push({
                      Store_Name,
              ...
              this.TechFixedTrendTotalsData.forEach((e: any) => {
                const subCategories = e.sub.reduce((r: any, { paytype }: any) => {
                  if (!r.some((o: any) => o.paytype == paytype)) {
                    r.push({
                      paytype,
              ...
              if (this.ReportTotal == 'B') {
                this.TechFixedTrendingData.push(this.TechFixedTrendTotalsData[0]);
              } else if (this.ReportTotal == 'T') {
                this.TechFixedTrendingData.unshift(
                  this.TechFixedTrendTotalsData[0]
                );
              }
              console.log('Service Trend Data', this.TechFixedTrendingData);
              this.shared.spinner.hide();
              if (this.TechFixedTrendingKeys && this.TechFixedTrendingKeys.length > 0) {
                this.NoData = false;
                this.shared.spinner.hide();
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.comm.pageName == 'Tech Fixed Trend Details') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        ...
        if (res.obj.title == 'Tech Fixed Trend Details') {
  ...
  openGraph(monthname: any, dates: any, Obj: any, SummaryType: any) {
    ...
    const STRgraph = this.shared.ngbmodal.open(TechFixedTrendGraph, {
      size: 'xl',
      backdrop: 'static',
    });
    if (Obj.variable == 'GP') {
  ...
  exportAsXLSX(): void {
    ...
    const worksheet = workbook.addWorksheet('Tech Fixed Trend Details');
    worksheet.views = [
      {
        state: 'frozen',
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Tech Fixed Trend Details']);
    ...
    const TechFixedTrendingData = this.TechFixedTrendingData.map(
      (_arrayElement: any) => Object.assign({}, _arrayElement)
    );
    const StartDate = this.shared.datePipe.transform(
      this.TechFixedTrendingKeys[1],
      'MMM yyyy'
    );
    const lastIndex = this.TechFixedTrendingKeys.length;
    const EndDate = this.shared.datePipe.transform(
      this.TechFixedTrendingKeys[this.TechFixedTrendingKeys.length - 1],
      'MMM yyyy'
    );
    const Header = [StartDate + ' - ' + EndDate];
    for (let i = 0; i < this.TechFixedTrendingKeys.length; i++) {
      Header.push(this.TechFixedTrendingKeys[i]);
    }
    ...
    for (const d of TechFixedTrendingData) {
      ...
      if (d.sub != undefined) {
        for (const d1 of d.sub) {
          ...
          for (const d2 of d1.subcategory) {
            ...
            for (const item of this.TechFixedTrendingKeys) {
    ...
    worksheet.addRow([]);
      ...
      this.shared.exportToExcel(workbook, 'Tech Fixed Trend Details'  );
