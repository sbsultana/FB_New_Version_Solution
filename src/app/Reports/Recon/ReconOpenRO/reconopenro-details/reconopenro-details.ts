...
@Component({
  selector: 'app-reconopenro-details',
   imports: [SharedModule,Notes],
  templateUrl: './reconopenro-details.html',
  styleUrl: './reconopenro-details.scss',
})
export class ReconopenroDetails {
  ...
  details: any = [];
  viewRO(roData: any) {
    // const modalRef = this.shared.ngbmodal.open(RepairOrderComponent, { size: 'md', windowClass: 'connectedmodal' });
        ...
        if (res.status == 200) {
          ...
          if (res.response != undefined) {
            if (res.response.length > 0) {
              ...
              const worksheet = workbook.addWorksheet('Recon Open RODetails');
              worksheet.views = [
                {
                  state: 'frozen',
              ...
              const titleRow = worksheet.getCell("A2"); titleRow.value = 'Recon Open RODetails';
              ...
              worksheet.addRow([]);
                ...
                this.shared.exportToExcel(workbook, 'Recon Open RODetails' + DATE_EXTENSION );
