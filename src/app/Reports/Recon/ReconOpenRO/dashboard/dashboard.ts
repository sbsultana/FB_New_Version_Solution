...
import { ReconopenroDetails } from '../reconopenro-details/reconopenro-details';

@Component({
  selector: 'app-dashboard',
  imports: [SharedModule, BsDatepickerModule, DateRangePicker, ReconopenroDetails, NgxSliderModule],
...
export class Dashboard {
  ...
  stores = [
    { "storename": "DOYLESTOWN RECON", "ID": 16, "storecode": "FB-A", "sg_id": 7, "sg_name": "Recon Centers" },
    { "storename": "LANGHORNE RECON", "ID": 43, "storecode": "MCL-A", "sg_id": 7, "sg_name": "Recon Centers" }
  ]
  ...
  constructor(
    ...
    this.shared.setTitle(this.comm.titleName + '-Recon Open RO');
  ...
  ngAfterViewInit() {
    ...
    //   if (this.comm.pageName == 'Recon Open RO') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      if (this.excel != undefined) {
        if (res.obj.title == 'Recon Open RO') {
  ...
  paytypeDisplay: any = ['Customerpay_0', 'Warranty_1', 'Internal_2'];
  ...
  individualStores(e: any, block: any) {
    if (block == 'All') {
  ...
  exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet('Recon Open RO');
    worksheet.views = [
      {
        showGridLines: false,
      },
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Recon Open RO']);
    ...
    this.shared.exportToExcel(workbook, 'Recon Open RO');
