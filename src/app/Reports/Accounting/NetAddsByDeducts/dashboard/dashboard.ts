...
export class Dashboard {
  ...
  exportAsXLSX(): void {
    ...
    const worksheet = workbook.addWorksheet('Net Adds / Deducts');
    worksheet.views = [
      {
        state: 'frozen',
        ySplit: 13,
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Net Adds / Deducts']);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Net Adds / Deducts' + EXCEL_EXTENSION);
    });
  }
 ...
 .d-flex {
  ...
  exportToExcel() {
    ...
    // Setup Excel
    const workbook = new Workbook();
    const worksheet = workbook.addWorksheet('Net Adds / Deducts Details');
    ...
    worksheet.addRow([]);
    const titleRow = worksheet.addRow(['Net Adds / Deducts Details']);
