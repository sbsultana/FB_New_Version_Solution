...
export class Dashboard {
  ...
  storesFilterData: any = {
    'groupsArray': this.groupsArray, 'groupId': this.groupId, 'storesArray': this.stores, 'storeids': '8', 'type': 'S', 'others': 'N',
  ...
  constructor(public shared: Sharedservice, public setdates: Setdates, private comm: common, private toast: ToastService,
    ...
    if (typeof window !== 'undefined') {
      ...
      this.shared.setTitle(this.shared.common.titleName + '-Used Car Wholesale');
  ...
  grouping: any = ['Used Car Wholesale Company']
  ...
  GetLenders(status: any, index: any) {
    ...
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetUsedCarWholesale', obj).subscribe(
      (res) => {
        if (res.status == 200) {
          ...
          if (res.response != undefined) {
            if (res.response.length > 0) {
              if (status == '') {
                this.NoData = '';
                this.LendersData = res.response.map((v: any) => ({
                  ...v, subData: []
                }));
              } else {
                this.LendersData[index].subData = res.response
              }
  ...
  expandorcollapse(ind: any, data: any) {
    const index = this.expanddataarray.findIndex((i: any) => i == ind);
    (index >= 0) ? this.expanddataarray.splice(index, 1) : this.expanddataarray.push(ind);
    (index >= 0) ? '' : this.GetLenders(data.stat, ind);
  ...
  GetLendersDetails() {
    ...
    this.shared.api.postmethod(this.comm.routeEndpoint + 'GetLenderSummaryDetails', obj).subscribe(
      (res) => {
        if (res.status == 200) {
          this.loader = false;
          this.LendersDataDetails = [];
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.comm.pageName == 'Used Car Wholesale') {
    ...
    this.shared.api.GetReportOpening().subscribe((res) => {
      // // console.log(res);
      if (res.obj.Module == 'Used Car Wholesale') {
    ...
    this.shared.api.getExportToExcelAllReports().subscribe((res) => {
      ...
      if (res.obj.title == 'Used Car Wholesale') {
