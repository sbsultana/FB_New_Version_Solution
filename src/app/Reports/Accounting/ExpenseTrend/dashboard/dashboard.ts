...
export class Dashboard {
  ...
  GetDataByMonths(date: any, filters: any) {
    ...
    console.log(obj);
    const curl = environment.apiUrl + 'WesternAuto/GetExpenseTrendReport';
    this.apiSrvc
    ...
    this.subscriptionReport = this.apiSrvc
      ...
      .subscribe((res) => {
        ...
        if (this.subscriptionReport != undefined) {
          if (res.obj.Module == 'Expense Trend') {
    ...
    this.subscriptionExcel = this.apiSrvc
      ...
      .subscribe((res) => {
        ...
        if (this.subscriptionExcel != undefined) {
          if (res.obj.title == 'Expense Trend') {
  ...
  exportAsXLSX(): void {
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Expense Trend']);
...
.notes { 
  ...
  <body id='content'>
    ...
    html2canvas(div, options)
      ...
      .then((doc) => {
        doc.save('Expense Trend Details.pdf');
        // popupWin.close();
        this.spinner.hide();
      });
  }
