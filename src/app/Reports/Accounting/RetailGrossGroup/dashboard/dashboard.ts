...
export class Dashboard {
  ...
  constructor(public apiSrvc: Api, private ngbmodalActive: NgbActiveModal,
    ...
    this.shared.setTitle(this.shared.common.titleName + '-Retail Gross Group')
  ...
  GetReconAnalysis(status: any, store: any, account: any, j: any) {
    ...
    const obj = {
      ...
      "Dept": this.Department == 'All' ? '' : this.Department

    };
    this.apiSrvc.postmethod(this.shared.common.routeEndpoint + 'GetGLRetailGrossGroup', obj).subscribe(
      (res) => {
        if (res.status == 200) {
  ...
  getReportFilters(): { title: string; filters: any[] } {
    ...
    return {
      ...
      filters: [
        ...
        {
          label: 'Department',
          value: this.Department && this.Department !== ''
            ? this.Department
            : 'All'
        }
      ]
  ...
  exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet("Retail Gross Group ");
    ...
    worksheet.addRow([]);
    const titleRow = worksheet.addRow(["Retail Gross Group "]);
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, `Retail Gross Group_${DATE_EXTENSION}.xlsx`);
    });
  }
}
