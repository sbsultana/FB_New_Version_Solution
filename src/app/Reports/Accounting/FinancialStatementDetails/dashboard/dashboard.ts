...
export class Dashboard {
    ...
    this.subscriptionExcel = this.apiSrvc
      ...
      .subscribe((res) => {
        ...
        if (this.subscriptionExcel != undefined) {
          if (res.obj.title == 'Financial Statement Details') {
  ...
  exportAsXLSX(): void {
    ...
    const worksheet = workbook.addWorksheet('Financial Statement Details');
    worksheet.views = [
      {
        state: 'frozen',
        ySplit: 13,
    ...
    worksheet.addRow('');
    const titleRow = worksheet.addRow(['Financial Statement Details']);
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Financial Statement Details' + EXCEL_EXTENSION);
    });
  }
 ...
 .d-flex {
  ...
  <body id='content'>
    ...
    html2canvas(div, options)
      ...
      .then((doc) => {
        doc.save('Financial Statement Details.pdf');
        // popupWin.close();
        this.spinner.hide();
      });
  }
...
.notes { 
  ...
  <body id='content'>
    ...
    html2canvas(div, options)
      ...
      .then((doc) => {
        doc.save('Financial Statement Details Details.pdf');
        // popupWin.close();
        this.spinner.hide();
      });
  }
