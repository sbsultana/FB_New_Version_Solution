...
export class Dashboard {
  ...
  LiabilitiesData: any = [
    {
      Liabilities: 'Aftermarkets',
      path: 'Aftermarkets A/P',
      title: 'Aftermarkets',
      url: 'Aftermarkets'
    },
    {
      Liabilities: 'Cert pay',
  ...
  constructor(
    ...
    if (path === 'Finance Product Liabilities') {
      this.selectedreceviabe = this.LiabilitiesData[0];
    } else {
    ...
    this.shared.setTitle(this.comm.titleName + '-Finance Product Liabilities');

    /* --------- HEADER FOR REPORTING --------- */
    const data = {
      title: 'Finance Product Liabilities',
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.shared.common.pageName == 'Finance Product Liabilities') {
    ...
    this.shared.api.GetReportOpening().subscribe((res) => {
      if (res.obj.Module === 'Finance Product Liabilities') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'Finance Product Liabilities' && res.obj.state == true) {
  ...
  exportToExcel(): void {
    ...
    const reportTitle =
      this.selectedreceviabe?.title
        ? `${this.selectedreceviabe.title} Finance Product Liabilities`
        : 'Finance Product Liabilities';
    ...
    const filters = [
      ...
      { label: 'Finance Product Liabilities', value: this.selectedreceviabe?.Liabilities || '-' },
    ];
    ...
    if (this.FloorPlanData?.length) {
      ...
      const agingValueRow = summarySheet.addRow([
        `${this.selectedreceviabe?.title || 'Finance Product Liabilities'} Aging`,
        A?.TOTAL ?? null,
        A?.D1 ?? null,
        A?.D2 ?? null,
        A?.D3 ?? null,
    ...
    workbook.xlsx.writeBuffer().then(() => {
      const rawFileName =
        `${this.selectedreceviabe?.title || 'Finance Product Liabilities'}_Liabilities_Report`;
