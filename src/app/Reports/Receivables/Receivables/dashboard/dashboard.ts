...
export class Dashboard {
  ...
  ngAfterViewInit(): void {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'Receivables' && res.obj.state == true) {
  ...
  exportToExcel(): void {
    ...
    const reportTitle =
      this.selectedreceviabe?.title
        ? `${this.selectedreceviabe.title} Receivables`
        : 'Receivables';
    ...
    const filters = [
      ...
      { label: 'Receivables', value: this.selectedreceviabe?.Receivables || '-' },
    ];
    ...
    workbook.xlsx.writeBuffer().then(() => {
      const rawFileName =
        `${this.selectedreceviabe?.title || 'Receivables'}_Receivables_Report`;
