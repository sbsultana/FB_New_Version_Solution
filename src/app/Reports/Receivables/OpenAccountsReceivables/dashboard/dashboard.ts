...
export class Dashboard {
  ...
  ngAfterViewInit(): void {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'Open Account Receivables' && res.obj.state == true) {
  ...
  exportToExcel(): void {
    ...
    const reportTitle =
      this.selectedreceviabe?.title
        ? `${this.selectedreceviabe.title} Open Account Receivables`
    ...
    const filters = [
      ...
      { label: 'Open Account Receivables', value: this.selectedreceviabe?.Liabilities || '-' },
    ];
    ...
    if (this.FloorPlanData?.length) {
      ...
      const agingValueRow = summarySheet.addRow([
        `${this.selectedreceviabe?.title || 'Open Account Receivables'} Aging`,
        A?.TOTAL ?? null,
        A?.D1 ?? null,
        A?.D2 ?? null,
        A?.D3 ?? null,
    ...
    workbook.xlsx.writeBuffer().then(() => {
      const rawFileName =
        `${this.selectedreceviabe?.title || 'Open Account Receivables'} Title Tracking_Report`;
