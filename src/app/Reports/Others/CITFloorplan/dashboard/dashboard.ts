...
export class Dashboard implements OnInit {
  ...
  constructor(
    ...
    // set page title
    try {
      this.shared.setTitle(this.shared.common.titleName + '-CIT/Floorplan');
    } catch {
      // fallback: no-op
    }
  ...
  ngAfterViewInit(): void {
    this.shared.api.getStores().subscribe((res: any) => {
      if (this.shared.common.pageName == 'CIT/Floorplan') {
    ...
    this.reportGetting = this.shared.api.GetReports().subscribe((data: any) => {
      if (data && data.obj && data.obj.Reference == 'CIT/Floorplan') {
    ...
    this.excel = this.shared.api.getExportToExcelAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'CIT/Floorplan' && res.obj.state == true) {
    ...
    this.print = this.shared.api.getExportToPrintAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'CIT/Floorplan' && res.obj.statePrint == true) {
        this.printPDF();
      }
    });

    this.Pdf = this.shared.api.getExportToPDFAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'CIT/Floorplan' && res.obj.statePDF == true) {
        this.generatePDF();
      }
    });

    this.email = this.shared.api.getExportToEmailPDFAllReports().subscribe((res: any) => {
      if (res && res.obj && res.obj.title == 'CIT/Floorplan' && res.obj.stateEmailPdf == true) {
  ...
  //     if (res && res.obj && res.obj.title == 'CIT/Floorplan' && this.Performance == 'Load') {
  ...
  //   this.AllStores = true;
  //   this.service.GetHeaderData().subscribe((res: any) => {
  //     if (res && res.obj && res.obj.title == 'CIT/Floorplan' && this.Performance == 'Load' && this.groupstate == false) {
...
this.shared.api.postmethod(this.shared.common.routeEndpoint + 'AddScheduleNotesAction', obj).subscribe((res: any) => {
  ...
  exportToExcel(): void {
    ...
    if (!workbook) {
      alert('Workbook helper not available in shared service.');
      return;
    }

    // --- Dashboard Summary sheet ---
    const summarySheet = workbook.addWorksheet('Dashboard Summary');
    summarySheet.addRow(['CITFloorplan']);

    let filters: any = [
      { name: 'Store :', values: 'WesternAuto' },
      { name: 'Deal Status :', values: this.clean(this.dealStatus) },
    ...
    const headers = [
      'CIT Age', 'Date', 'Account', 'Control', 'Control 2',
      'CIT', '	Floorplan	', 'Customer', 'Number	', 'Stock #', '	Deal #','Stage','F&I Mgr','Sales Mgr',
    ...
    workbook.xlsx.writeBuffer().then((buffer: any) => {
      try {
        ...
        if (this.shared.exportToExcel) {
          this.shared.exportToExcel(workbook, 'CIT_Floorplan_Report');
        } else {
          // fallback: use FileSaver if available globally (not implemented here)
          alert('Export helper not found. Implement shared.exportToExcel or add FileSaver logic.');
        }
      } catch (err) {
